...
const NasdaqPrediction = () => {
  ...
  // Cache key for NASDAQ data
  const CACHE_KEY = 'kalshi_nasdaq_prediction';
  ...
  const fetchPredictionData = async () => {
    try {
      ...
      const response = await fetch(ENDPOINTS.NASDAQ_PREDICTION, {
  ...
  const processKalshiData = (data) => {
    ...
    return {
      midpoint: midpointContract?.title || '16500',
